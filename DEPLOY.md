...
- **pnpm** (this app uses it): must be on your PATH for the whole deploy — the CDK Lambda bundler invokes `pnpm` by name while bundling, so a one-off `npx pnpm` isn't enough. Enable it with `corepack enable` (ships with Node), then verify `pnpm --version`.
...
`cdk deploy` builds the frontend (`pnpm run build` → `artifacts/madlibs-client/dist/public/`) and uploads the output to S3 + CloudFront automatically. Do **not** run `aws s3 sync` afterwards — the Hosting construct handles asset upload and cache invalidation.
...
- Check that `pnpm run build` produces output in the `artifacts/madlibs-client/dist/public/` directory
